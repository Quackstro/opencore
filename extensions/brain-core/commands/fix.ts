/**
 * Brain Core — /fix command handler.
 *
 * Corrections supported:
 *   /fix <id> → <bucket>       Move item to a bucket
...
function formatRecord(
  ...
  lines.push(`  → <bucket>  — move (buckets: ${buckets.join(", ")})`);
  ...
  switch (action.type) {
    ...
    case "move": {
      ...
      if (!targetBucket) {
        return {
          success: false,
          message: `❌ Unknown bucket: "${action.target}". Valid: ${configuredBuckets.join(", ")}`,
          itemId: id,
        };
      }
